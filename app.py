...
@app.route('/')
def home():
    return 'Hello, this is your Flask app with SQLAlchemy!'


def load_clientes():
    for _ in range(100):
        cliente = ClienteModel(nombre=fake.first_name(),
                               apellido=fake.last_name(), email=fake.email())
    ...
    with open('./docs/equipo.csv', encoding='utf-8') as csv_file:
        try:
            ...
            for row in csv_reader:
                equipo = EquipoModel(
                    nombre=row[0], escudo=row[1], pais=row[2], puntaje=float(row[3]))
    ...
    with open('./docs/partidos.csv', encoding='utf-8') as csv_file:
        ...
        for row in csv_reader:
            partido = PartidoModel(
                equipo_local_id=row[1], equipo_visitante_id=row[2])
...
@app.route('/view-clientes')
def view_clientes():
    clientes = ClienteModel.query.all()
    return render_template('view_clientes.html', clientes=clientes)
