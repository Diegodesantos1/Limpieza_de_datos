...
for i in range(len(datos)):
    ...
    df_trabajar.to_csv("Datos_Limpios/UCL" + nombres[i] + ".csv", index=False)
...
for i in range(len(datos_limpios)):
    df = pd.read_csv(datos_limpios[i])
    df['Home Team'] = df['Home Team'].replace(['Roma', 'AS Roma'], 'AS Roma')
    df['Away Team'] = df['Away Team'].replace(['Roma', 'AS Roma'], 'AS Roma')
    df.to_csv(datos_limpios[i], index=False)
...
for i in range(len(datos_limpios)):
    df = pd.read_csv(datos_limpios[i])
    df['Home Team'] = df['Home Team'].replace(['PSV', 'PSV Eindhoven'], 'PSV')
    df['Away Team'] = df['Away Team'].replace(['PSV', 'PSV Eindhoven'], 'PSV')
    df.to_csv(datos_limpios[i], index=False)
...
# Voy a crear un sistema de puntuación para los equipos dando 3 puntos por victoria y 1 por empate

puntos = {}
for equipo in equipos:
    puntos[equipo] = 0

# Ahora voy a recorrer todos los datos y voy a sumar los puntos de cada equipo
for i in range(len(datos_limpios)):
    df = pd.read_csv(datos_limpios[i])
    for index, row in df.iterrows():
        if row['Winner'] == row['Home Team']:
            puntos[row['Home Team']] += 3
        elif row['Winner'] == row['Away Team']:
            puntos[row['Away Team']] += 3
        else:
